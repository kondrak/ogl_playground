...
void processEvents()
{
    ...
    while( SDL_PollEvent( &event ) )
    {
        ...
        case SDL_MOUSEBUTTONDOWN:
            g_application.OnMouseDown( event.button.button, event.button.x, event.button.y );
            break;
        case SDL_MOUSEBUTTONUP:
            g_application.OnMouseUp( event.button.button, event.button.x, event.button.y );
            break;
        case SDL_MOUSEWHEEL:
            g_application.OnMouseWheel( event.wheel.x, event.wheel.y );
            break;
        case SDL_QUIT:
            g_application.Terminate();
            break;
        ...
        case SDL_WINDOWEVENT:
            ...
            case SDL_WINDOWEVENT_RESIZED:
                g_application.OnWindowSizeChanged( event.window.data1, event.window.data2 );
                break;
            }
            break;
        }
